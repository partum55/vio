...
void Viewer::record(const RecordConfig& cfg) {
    ...
    // Create offscreen Pangolin window
    ...
    const double fx = 0.7 * W;  // focal length in pixels
    ...
    for (int pose_idx = 0; pose_idx < total_poses; pose_idx += cfg.poses_per_frame) {
        ...
        // Agent's backward direction (camera looks along -Z in its frame)
        ...
        // Read pixels (OpenGL origin is bottom-left, flip vertically)
        ...
        if (frame % 30 == 0) {
            std::cout << "  frame " << frame << " / ~" << total_poses / cfg.poses_per_frame << "\r" << std::flush;
        }
    }
...
void Viewer::drawCurrentFrustum(int pose_idx) {
    if (pose_idx < 0 || pose_idx >= static_cast<int>(trajectory_.size())) return;

    const float w = frustum_scale_ * 2.0f;  // larger than keyframe frustums
    ...
    glLineWidth(2.5f);
    glColor3f(1.0f, 0.9f, 0.2f); // bright yellow for current pose

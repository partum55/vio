...
void trackPointsLKSingleLevel(
    ...
    if (imgPrevGray.empty() || imgCurrGray.empty())
        throw std::runtime_error("trackPointsLKSingleLevel: empty input image");

    const cv::Mat imgPrevF = toFloatGray(imgPrevGray);
    const cv::Mat imgCurrF = toFloatGray(imgCurrGray);

    cv::Mat IxCurr, IyCurr;
    computeGradients(imgCurrF, IxCurr, IyCurr);

...
bool ImuTrackingPipeline::runTrackingAndSync()
{
    ...
    vio::ThreadPool pool(num_threads);
    vio::CustomShiTomasiDetector detector(pool, num_threads);

    vio::ShiTomasiParams shi_params;
    shi_params.maxCorners = 100;
    shi_params.qualityLevel = 0.01;
    shi_params.minDistance = 10.0;

    std::vector<cv::Point2f> initial_pts = detector.detect(prev_gray, shi_params);
    ...
    for (size_t img_idx = 1; img_idx < image_paths_.size(); ++img_idx) {
        ...
        refreshTracksIfNeeded(curr_gray, tracks, next_track_id, refresh_params, detector);
